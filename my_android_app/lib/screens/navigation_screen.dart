...
class _NavigationScreenState extends State<NavigationScreen> {
  ...
  void _goToOneDayView(BuildContext context) async {
    ...
    rows.sort((a, b) {
      final dateA = DateTime.tryParse(a['日付']!.replaceAll('/', '-')) ?? DateTime(1970);
      final dateB = DateTime.tryParse(b['日付']!.replaceAll('/', '-')) ?? DateTime(1970);
      return dateB.compareTo(dateA);
    });

    final selectedRow = rows.first;
    final selectedDate = DateTime.tryParse(selectedRow['日付']!.replaceAll('/', '-'));
